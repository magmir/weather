$(document).ready(function() {
  ...
  $(function() { 
    ...
    if (localStorage.getItem('current')) {
      $('#city').val(localStorage.getItem('current')).trigger('change');
    }
  });
  ...
  $('button').on('click', function() {
    ...
    $.ajax({
      ...
      success: function weather(data) {
        ...
        var html = 'Location: ' + data.name + '<br>' + 'Temperature: ' + temp + 'C<br>' + 'Humidity: ' + data.main.humidity + '%<br>' + 'Wind: ' + data.wind.speed + 'km/h \n';
        $('#weather').html(html);
       // html = '';
